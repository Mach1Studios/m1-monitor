...
/// Logic flow for determining if M1-MONITOR will be a dynamic I/O plugin that can change its input/output configurations
...
#pragma message "Value of OUTPUTS: " XSTR(JucePlugin_MaxNumOutputChannels)
    ...
    // Please add in jucer definitions or in cmake via `ADD_DEFINITIONS(-DSTREAMING_MONITOR_PLUGIN)`
    #ifdef STREAMING_MONITOR_PLUGIN
        /// STREAMING_MONITOR_PLUGIN mode active
    #else
...
#pragma message "Value of OUTPUT_CHANNELS: " XSTR(OUTPUT_CHANNELS)
    ...
    #undef DYNAMIC_IO_PLUGIN_MODE
    #undef STREAMING_MONITOR_PLUGIN
...
#if defined(CUSTOM_CHANNEL_LAYOUT) && (!defined(CUSTOM_CHANNEL_LAYOUT_BY_HOST) || !defined(DYNAMIC_IO_PLUGIN_MODE) || !defined(STREAMING_MONITOR_PLUGIN))
    #pragma message "CUSTOM_CHANNEL_LAYOUT Active"
#elif defined(CUSTOM_CHANNEL_LAYOUT_BY_HOST) && (!defined(CUSTOM_CHANNEL_LAYOUT) || !defined(DYNAMIC_IO_PLUGIN_MODE) || !defined(STREAMING_MONITOR_PLUGIN))
    #pragma message "CUSTOM_CHANNEL_LAYOUT_BY_HOST Active"
#elif defined(DYNAMIC_IO_PLUGIN_MODE) && (!defined(CUSTOM_CHANNEL_LAYOUT) || !defined(CUSTOM_CHANNEL_LAYOUT_BY_HOST) || !defined(STREAMING_MONITOR_PLUGIN))
    #pragma message "DYNAMIC_IO_PLUGIN_MODE Active"
#elif defined(STREAMING_MONITOR_PLUGIN) && (!defined(CUSTOM_CHANNEL_LAYOUT) || !defined(CUSTOM_CHANNEL_LAYOUT_BY_HOST) || !defined(DYNAMIC_IO_PLUGIN_MODE))
    #pragma message "STREAMING_MONITOR_PLUGIN Active"
#else
